...
CardRouter.post("/", upload.single("portada"), checkToken, async (req, res) => {
  try {
    if (!req.file || !type || !title || !writer || !editorial) {
...
CardRouter.get("/", checkToken, async (req, res) => {
  try {
    const cards = await Card.find({}).select(
      "title portada number writer editorial language publication_Date"
    );
    return res.send({
      success: true,
      cards,
...
CardRouter.get("/filters", checkToken, async (req, res) => {
  try {
    ...
    let filter = await Card.find(query).select(
      "title number writer editorial publication_Date"
    );
    /* .populate("type")
        .populate("title")
        .populate("number")
